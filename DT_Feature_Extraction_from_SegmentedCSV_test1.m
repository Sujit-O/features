function []= DT_Feature_Extraction_from_SegmentedCSV_test1(folderName)
...
%         '\segments*'));
    segmentChannelFolders={'segments_Floor';'segments_Wall'};
    ...
    for segmentFolder_i=1:size(segmentChannelFolders,1)
        segmentFiles={'segment_2'; 'segment_13'; 'segment_7'; 'segment_8'};
%         segmentFiles =dir(strcat(originFolder,...
%             '\',folderName,'\',totalChannelFolders(channel_i).name,...
%             '\',segmentChannelFolders(segmentFolder_i),...
%             '\segment*.csv'));
...
%         disp(segmentChannelFolders{segmentFolder_i});
        ...
        for segment_i=1:size(segmentFiles,1)
            ...
            creatFolder(strcat(destinationFolder,...
                '\',folderName,'\',totalChannelFolders(channel_i).name,'\',...
                segmentChannelFolders{segmentFolder_i}),...
                segmentFiles{segment_i});
            
%            disp(segmentFiles{segment_i});
...
%              Divide data into 5 parts to handle memory issues
            for kk=1:floor(length(data1)/5):(length(data1)-floor(length(data1)/5))
                data=data1(kk:(kk+floor(length(data1)/5)-1));
                ...
                features = DT_FeatureExtraction_Frequency_Test1(data,...
                ...
                features = DT_STFTStatistics_Test1(data, fs, STFT_win,...
