function []= DT_Feature_Extraction_from_SegmentedCSV_test(folderName)
...
destinationFolder =strcat('D:\GDrive\DT_Data\DAQ_Test');
...
%                 features = DT_FeatureExtraction_Time(data1, fs,...
...
%                 
%                 features = DT_FeatureExtraction_Frequency(data1,...
...
%                 features = DT_STFTStatistics(data1, fs, STFT_win,...
...
%                 dlmwrite(fileNameFullS,features,'-append','delimiter',',');
            ...
            for kk=1:floor(length(data1)/10):(length(data1)-floor(length(data1)/10))
                data=data1(kk:(kk+floor(length(data1)/10)-1));
%                 %% Time Features Extraction
