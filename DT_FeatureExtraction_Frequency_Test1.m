function frequencyFeatures = DT_FeatureExtraction_Frequency_Test1(signal, fs, win, step, freqrange,harmonicNumber,numberOfMFCCCoef,param)
...
for i=1:numberOfFrames
    ...
    frame  = frame .* Ham;
    frameFFT = F_Frequency_Feature_getDFT_noGPU(frame, fs);
...
%             i)                    = F_Frequency_Feature_OrderSpectrum(frame, fs, harmonicNumber);    fnum=fnum+orderedSpectrumFeatureNumber;
        frequencyFeatures(fnum:fnum+numberOfMFCCCoef-1,...
            i)                    = F_Frequency_Feature_MFCCs_Test1(frame, fs);                       fnum=fnum+numberOfMFCCCoef;
